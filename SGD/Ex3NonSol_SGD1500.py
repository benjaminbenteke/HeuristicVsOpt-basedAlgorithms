...
#   bound1 = [(None,b-1.0)]
#   bound2 = [(None,a-1.0)]
  constraint1 = [{'type': 'ineq', 'fun': g11}, {'type': 'ineq', 'fun': g12}]
  constraint2 = [{'type': 'ineq', 'fun': g21}, {'type': 'ineq', 'fun': g22}]
#   , bounds= bound1, bounds= bound2
  result1 = minimize(func1, x0=0.0, method='SLSQP', constraints= constraint1,bounds= bound1) #, options={'maxiter':5}
...
def run_example(n_parents):
    ...
    def pen_children(p,sigma,P):
        ...
        penK= Parallel(n_jobs=num_processes)(delayed(pen)(children[i][0], children[i][1]) for i in range(n_children))
        return penK
    ...
    def get_sig(num):
        return np.random.normal(loc = 0.0, scale = 0.99 ** num, size= n_children)
...
def run_parallel(num_points, n_r):
    ...
    for i in range(n_r):
        np.savetxt(f'./Ex3/N_{num_points}/{i}_solns_run_{10}_{num_points}pts.txt', results[i], delimiter=',')
