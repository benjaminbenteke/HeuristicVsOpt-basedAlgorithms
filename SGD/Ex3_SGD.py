...
def pen(a,b):
  ...
  def func2(x):
    return x**2 - 0.5*x*a - 2*x

 # b1 = math.sqrt(1.0 - (b)**2)
 # b2 = math.sqrt(1.0 - (a)**2)

  min1 = gold(func1,0.0001,math.sqrt(abs(1.0-(b)**2)))
  min2 = gold(func2,0.0001,math.sqrt(abs(1.0-(a)**2)))
  #print(min1)
  shadowX = min1
  shadowY = min2
...
number_points_list = [900, 1000, 1100, 1200, 1300, 1400, 1500]
n_runs = [10]
n_r= 10
def run_parallel(num_points,n_r):
    results = Parallel(n_jobs=-1,prefer= 'threads')(delayed(run_each)(num_points) for _ in range(n_r))
    for i in range(n_r):
        np.savetxt('./solns_runs/Ex3/N_' + str(num_points) + "/" + str(i + 1) + "_" + "solns" + '_' + 'run_' + str(n_runs[0]) + '_' + str(num_points) + 'pts' + '.txt', results[i], delimiter=',')
