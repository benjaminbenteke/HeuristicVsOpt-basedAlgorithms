...
for p in percent_list:
    ...
    for num_points in nubmer_points_list:
        ...
        path = f"{base_folder}/{p}/Ex1/N_{num_points}"
...
#   bound2 = [(0.0,1.0)]
  ...
  result2 = minimize(func2, x0=0.0, method='SLSQP', bounds = bound2,constraints= constraint2)#, options={'maxiter':5})
...
def run_with_diff_n_runs(perc, num_points):
    ...
    for n_r in n_runs:
        ...
        for i in range(n_r):
            ...
            res= np.array(res)
            np.savetxt(
                f'./percent/{perc}/Ex1/N_{num_points}/'
...
# def run_for_percent(percent):

#     print(f"Running experiments for {percent}% deletion")

#     results = Parallel(n_jobs=num_processes)(delayed(run_with_diff_n_runs)(percent, num_points) for num_points in nubmer_points_list)

# # Run all percentages in parallel
# Parallel(n_jobs=num_processes)(
#     delayed(run_for_percent)(percent)
