...
# Define objective functions for each player
def player1_objective(x1, x2):
    return x1**2+(8/3)*x1*x2-34*x1

def player2_objective(x2, x1):
    return x2**2+(5/4)*x1*x2-24.25*x2

# Define constraints for each player
def player_constraint11(x1, x2):
    return 15-x1-x2 # Example: -2 * x1 + x2 - 2 * x3 <= 5
...
def update_player_strategy(player_index, current_strategies):
    ...
    if player_index == 0:
        ...
        result = minimize(objective_function, x0= 0.0, constraints=constraint, bounds= [(0.0,10.0)],method='SLSQP')
        min_x_i = result.x
    else:
        other_players_strategies = np.delete(current_strategies, player_index, axis=0)
        ...
        result = minimize(objective_function, x0=0.0, constraints=constraint, bounds= [(0.0,10.0)], method='SLSQP') # , constraints=constraint
        min_x_i = result.x[0]
...
num_players = 2
final_strategies = gauss_seidel_gnep(num_players)
print("Final Strategies:", final_strategies)
