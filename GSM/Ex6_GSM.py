...
def player_constraint1(x1, x2):
    return 1-(x1+x2) # Example: -2 * x1 + x2 - 2 * x3 <= 5
def player_constraint2(x2, x1):
    return 1-(x1+x2)
...
def update_player_strategy(player_index, current_strategies):
    ...
    if player_index == 0:
        ...
        constraint = [{'type': 'ineq', 'fun': lambda x: player_constraint1(x, other_players_strategies)}]
        result = minimize(objective_function, x0= 0.0, constraints=constraint, bounds= bound)
        min_x_i = result.x
    else:
        # other_players_strategies = current_strategies[:2]
        other_players_strategies = np.delete(current_strategies, player_index, axis=0)
        ...
        constraint = [{'type': 'ineq', 'fun': lambda x: player_constraint2(x,other_players_strategies)}]
        result = minimize(objective_function, x0=0.0, constraints=constraint, bounds= bound) # , constraints=constraint
        min_x_i = result.x[0]
